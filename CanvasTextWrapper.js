...
(function() {
  ...
  var defaults = {
    ...
    background: 'white' // background color of the canvas
  };
  ...
  var CanvasTextWrapper = function(canvas, text, options) {
    ...
    for (var property in defaults) {
      if (defaults.hasOwnProperty(property)) {
        this[property] = (options && options[property]) ? options[property] :
          defaults[property];
      }
    }

    EL_WIDTH = (this.fitParent === false) ? this.canvas.width : this.canvas
      .parentNode
      .clientWidth;
    EL_HEIGHT = (this.fitParent === false) ? this.canvas.height : this.canvas
      .parentNode
      .clientHeight;
  ...
  CanvasTextWrapper.prototype = {
    _init: function() {
      ...
      this.fontSize = this.font.match(/\d+(px|em|\%)/g) ? +this.font.match(
        /\d+(px|em|\%)/g)[0].match(/\d+/g) : 18;
    ...
    _render: function() {
      if (this.sizeToFill) {
        ...
        do {
          ...
          this._wrap();
        } while (this.textBlockHeight < MAX_TXT_HEIGHT && (this.lines.join(
            ' ').split(
            /\s+/).length == numWords));
    ...
    _setFont: function(fontSize) {
      var fontParts = (!this.sizeToFill) ? this.font.split(/\b\d+px\b/i) :
        this
    ...
    _checkLength: function(words) {
      ...
      for (var i = 0; i < words.length; i++) {
        ...
        if (tokenLen > MAX_TXT_WIDTH) {
          for (var k = 0;
            (this.context.measureText(testString + words[i][k]).width <=
              MAX_TXT_WIDTH) && (k < words[i].length); k++) {
    ...
    _breakText: function(words) {
      for (var i = 0, j = 0; i < words.length; j++) {
        ...
        if (this.lineBreak === 'auto') {
          while ((this.context.measureText(this.lines[j] + words[i]).width <=
              MAX_TXT_WIDTH) && (i < words.length)) {
    ...
    _justify: function() {
      ...
      for (i = 0; i < this.lines.length; i++) {
        ...
        numOfSpaces = (maxLen - this.context.measureText(this.lines[i]).width) /
          spaceLength;
    ...
    _drawText: function() {
      for (var i = 0; i < this.lines.length; i++) {
        ...
        this.context.fillText(this.lines[i], this.textPos.x, this.textPos
          .y);

        if (this.strokeText) {
          this.context.strokeText(this.lines[i], this.textPos.x, this.textPos
            .y);
        }
      }
    },
    ...
    _setAlignX: function(line) {
      if (this.textAlign == 'center') {
        this.textPos.x = (EL_WIDTH - this.context.measureText(line).width) /
          2;
    ...
    _validate: function() {
      if (!(this.canvas instanceof HTMLCanvasElement))
        throw new TypeError(
          'The first parameter must be an instance of HTMLCanvasElement.'
        );
      ...
      if (typeof this.background !== 'string')
        throw new TypeError('Background color must be a string.');
      ...
      if (this.textAlign.toLocaleLowerCase() !== 'left' && this.textAlign
        .toLocaleLowerCase() !==
        'center' && this.textAlign.toLocaleLowerCase() !== 'right')
        throw new TypeError(
          'Property "textAlign" must be set to either "left", "center", or "right".'
        );

      if (this.verticalAlign.toLocaleLowerCase() !== 'top' && this.verticalAlign
        .toLocaleLowerCase() !== 'middle' && this.verticalAlign.toLocaleLowerCase() !==
        'bottom')
        throw new TypeError(
      ...
      if (typeof this.justifyLines !== 'boolean')
        throw new TypeError(
          'Property "justifyLines" must be set to a Boolean.');

      if (isNaN(this.paddingX))
        throw new TypeError(
          'Property "paddingX" must be set to a Number.');

      if (isNaN(this.paddingY))
        throw new TypeError(
          'Property "paddingY" must be set to a Number.');

      if (typeof this.fitParent !== 'boolean')
        throw new TypeError(
          'Property "fitParent" must be set to a Boolean.');

      if (this.lineBreak.toLocaleLowerCase() !== 'auto' && this.lineBreak
        .toLocaleLowerCase() !==
        'word')
        throw new TypeError(
      ...
      if (typeof this.sizeToFill !== 'boolean')
        throw new TypeError(
          'Property "sizeToFill" must be set to a Boolean.');

      if (typeof this.strokeText !== 'boolean')
        throw new TypeError(
          'Property "strokeText" must be set to a Boolean.');
    }
  };
